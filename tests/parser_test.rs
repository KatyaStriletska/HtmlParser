...
fn thow_mismatched_closing_tag_error() -> Result<()> {
    ...
    match result {
        Ok(_) => Err(anyhow!("Test failed! There should be an error here due to tag mismatch.")),
...
fn invalid_html_structure() -> Result<()> {
    ...
    let result = parse_html(input);
    match result {
        Ok(_) => Err(anyhow!("Test failed! There should be an error here due to invalid structure.")),
    ...
    match result {
        Ok(elements) =>{
            ...
            match &elements[0] { 
                ...
                _ => return Err(anyhow!("Test failed! First Element of html dom must be documentatation tag."))
            }
            match &elements[1] {
                            ...
                            match &children[0] {
                                HtmlElem::Tag { tag_name, children } => {
                                    ...
                                    if let HtmlElem::Text(text) = &children[0] {
                                        ...
                                        return Err(anyhow!("Test failed! <p> should contain text."));
                                    }
                                }
                                _ => return Err(anyhow!("Test failed! The first child of <head> should be a <p> tag.")),
                            }
                            match &children[1] {
                                HtmlElem::Tag { tag_name, children } => {
                                ...
                                _ => return Err(anyhow!("Test failed! The second child of <head> should be a <br> tag.")),
                            }
                        }
                        _ => return Err(anyhow!("Test failed! The child of <html> should be a <head> tag.")),
                    }
